"""Correct epic-care.com Danville ZIP 94525 -> 94526 (site typo; 94525 is Crockett). Re-geocode,
clear zip_check, update validation. Targeted cell update, re-sync local."""
...
def main():
    ...
    for i, row in enumerate(sheet[1:], start=2):
        if row[0].strip().lower() == "epic-care.com" and row[1].strip() == "Danville":
            g = vg.google_status(f"{row[2]}, {row[3]}, {row[4]} {NEW_ZIP}")
            vs = "passed" if g == "exists" else "needs_review"
            zf = zc.flag(NEW_ZIP, row[3], row[4])
            cells += [gspread.cell.Cell(i, C_ZIP, NEW_ZIP), gspread.cell.Cell(i, C_VS, vs),
                      gspread.cell.Cell(i, C_ZC, zf)]
            print(f"  row {i} Danville -> zip {NEW_ZIP} | geocode={g} -> {vs} | zip_check='{zf}'")
            break
